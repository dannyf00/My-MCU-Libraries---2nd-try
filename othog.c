#include "othog.h"						//othogoal approach to detect rms + phase
...
//void de_DC a series
//not used
void dedc_f(float *dat, int block_size) {
	int i;
	float avg=0;
...
void othogrms_f(float *dat, float *rms, float *phase, int block_size) {
	int i;
...
void othog_f(float * volt, float *amp, float *impedance, float *phase, int block_size) {
	...
	othogrms_f(volt, &volt_rms, &volt_phase, block_size);	//calculate volt's rms and phase (vs. reference signal)
	othogrms_f(amp, &amp_rms, &amp_phase, block_size);		//calculate amp's rms and phase (vs. reference signal)
