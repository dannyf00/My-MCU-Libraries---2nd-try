...
#define	LCD_STROBE(rs)	{if (!rs) {LCD_CLR(LCD_RS); delay_2wi();} LCD_SET(LCD_EN); lcd_delay(); LCD_CLR(LCD_EN);}	//strobe out the data/control
...
void spi_send_byte(unsigned char c) {
    ...
    //NOP();
	LCD_SET(SPI_CLK);				//clk idles high
	delay_2wi();				//wait for clk / mosi to go high

	mask = 0x80;                	//Initialize to write and read bit 7

    do  {
		di();					//protect the timing
		LCD_CLR(SPI_CLK);
		if (c & mask) {/*do nothing*/}	//send 1 bit
		else {delay_2wi();}	//send 0 bit
		//lcd_delay();				//NOP(); NOP();
        ...
        delay_2wi();			//to insure clk is high
