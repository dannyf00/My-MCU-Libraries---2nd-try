...
#include "tmr32b1_pwm.h"			//we use tmr16b0
...
void tmr32b1_setdc1(uint32_t match/*, uint32_t config*/) {
	...
	LPC_IOCON->PIO0_7 = (LPC_IOCON->PIO0_7 & ~(0x07)) | 0x04;     //4 -> set up pin for PWM use (sec 7.4.23)
}
...
void tmr32b1_setdc2(uint32_t match/*, uint32_t config*/) {
	...
	//MAT2 on P0.8
	//output can be mapped to multiple pins, with differing masks (0x03 / 0x04)
	LPC_IOCON->PIO0_8 = (LPC_IOCON->PIO0_8 & ~(0x07)) | 0x04;     //4 -> set up pin for PWM use (sec 7.4.23)
}
...
void tmr32b1_setdc3(uint32_t match/*, uint32_t config*/) {
	...
	//MAT3 on P0.9
	//output can be mapped to multiple pins, with differing masks (0x03 / 0x04)
	LPC_IOCON->PIO0_9 = (LPC_IOCON->PIO0_9 & ~(0x07)) | 0x04;     //4 -> set up pin for PWM use (sec 7.4.23)
}
