...
void uart0_init(uint32_t bps) {
	...
	//configure the pins
    // Change P0.2/TXD0 and P0.1/RXD0 mode '0b010'
	LPC_IOCON->PIO0_2 = (LPC_IOCON->PIO0_2 &~(0b111 <<  0)) | (0b010 <<  0);	//'0b010'->P0.2 to txd0
	LPC_IOCON->PIO0_1 = (LPC_IOCON->PIO0_1 &~(0b111 <<  0)) | (0b010 <<  0);	//'0b010'->P0.1 to rxd0

	//enable power to uart
	LPC_SYSCON->SYSAHBCLKCTRL |= (1<<12);	//12->uart0, 13=uart1, 14=adc
	LPC_SYSCON->UART0CLKDIV = 4;			//set uart clock divider. 0->disable uart
	...
	//}
	F_UART = F_CPU / LPC_SYSCON->UART0CLKDIV;
