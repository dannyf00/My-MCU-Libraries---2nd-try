...
//pinconfiguration
#define UxTX2RP()			PPS_U1TX_TO_RP(6)			//map u1tx pin to an rp pin
#define UxRX2RP()			PPS_U1RX_TO_RP(1)			//map u1rx pin to an rp pin
//end pin configuration

//for U1ART
#define UxMD				_U1MD		//power management bit
#define UxMODE				U1MODEbits	//mode register
#define UxBRG				U1BRG		//baud rate generator
#define UxSTA				U1STAbits	//status register
#define UxTXIE				_U1TXIE		//interrupt enable bit
#define UxTXIF				_U1TXIF		//interrupt flag
#define UxRXIE				_U1RXIE		//interrupt enable bit
#define UxRXIF				_U1RXIF		//interrupt flag
...
void uart1_puts(char *str) {
	while(*str) {
		uart1_putch(*str++);	//send the ch and advance the pointer
	}
}
