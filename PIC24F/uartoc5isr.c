#include "uartoc5isr.h"							//we use softwar uart 1 on Output Compare
...
//use OC1..5
#define OCxMD				PMD2bits.OC5MD
#define OCxCON1				OC5CON1
#define OCxCON2				OC5CON2
#define OCxR				OC5RS
#define OCxTMR				OC5TMR
#define OCxIF				IFS2bits.OC5IF		//interrupt flag
#define OCxIE				IEC2bits.OC5IE		//interrupt enable
//end hardware configuration

//global defines
#define F_UART				F_CPU				//UART clock sourced from system clock

//not used -> system clock as time base
...
void uartoc5_init(uint32_t bps) {
	...
	OCxCON1 = 	(0<<13) |				//0->output compare continue in idle; 1->output compare disabled in idle
				...
				(0<< 0) |				//0->OC disabled, 1..5: OC enabled, 6..7: PWM mode
				0x00;
...
void uartoc5_put(char ch) {
	...
	uartoc5_puts(_buffer);						//send the buffer
}

//send a string
void uartoc5_puts(char *str) {
	char ch;
	while (uartoc5_busy()) continue;			//wait for the uart to be ready
	_UxTX_BUSY = 1;							//uart is busy
	//IO_SET(UART_PORT, UART_TX);			//TX high
	//reverse the input byte's order: LSB..MSB
	_UxTXphase = 1;							//to send the 1st byte
