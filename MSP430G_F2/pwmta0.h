...
#define PWM_TA0CH1()				do {IO_OUT(P1DIR, 1<<2); IO_SET(P1SEL, 1<<2); /*IO_AF1(PORT1, 1<<0);*/} while (0)	//CH1 output on P1.2, AF1
#define PWM_TA0CH2()				do {IO_OUT(P1DIR, 1<<3); IO_SET(P1SEL, 1<<3); /*IO_AF1(PORT1, 1<<1);*/} while (0)	//CH2 output on P1.3, AF1
...
void pwmta0_setdc1(uint16_t dc);	//TIMERA0->CCR1 = (dc)
void pwmta0_setdc2(uint16_t dc);	//TIMERA0->CCR2 = (dc)
void pwmta0_setdc3(uint16_t dc);	//TIMERA0->CCR3 = (dc)
void pwmta0_setdc4(uint16_t dc);	//TIMERA0->CCR4 = (dc)
void pwmta0_setdc5(uint16_t dc);	//TIMERA0->CCR5 = (dc)
void pwmta0_setdc6(uint16_t dc);	//TIMERA0->CCR6 = (dc)
