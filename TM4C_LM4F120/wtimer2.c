#include "wtimer2.h"									//use timer2

//hardware configuration
//end hardware configuration

//global defines
#define TMRx				WTIMER2					//current timer
...
void wtmr2a_init(uint32_t ps, uint32_t pr) {
	_isrptr_tmra = empty_handler;

	//route the clock
	SYSCTL->RCGCWTIMER |= (1<<2);						//1->route clock. TIMER0/1/2/3/4/5
...
void wtmr2a_act(void (*isrptr)(void)) {
	_isrptr_tmra = isrptr;
	TMRx->ICR |= (1<<0);							//1-.clera the flag, 0->no effect
	TMRx->IMR |= (1<<0);							//1->enable time-out interrupt, 0->disable it.
	NVIC_EnableIRQ(WTIMER2A_IRQn);					//use default priorities
}

//reset timer0b - split timer
//16/32-bit timer: 8-bit prescaler, 16-bit period register
void wtmr2b_init(uint32_t ps, uint32_t pr) {
