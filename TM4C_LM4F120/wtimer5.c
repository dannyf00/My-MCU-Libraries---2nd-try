#include "wtimer5.h"									//use timer5

//hardware configuration
//end hardware configuration

//global defines
#define TMRx				WTIMER5					//current timer
...
void wtmr5a_init(uint32_t ps, uint32_t pr) {
	...
	//route the clock
	SYSCTL->RCGCWTIMER |= (1<<5);						//1->route clock. TIMER0/1/2/3/4/5
...
void wtmr5a_act(void (*isrptr)(void)) {
	...
	NVIC_EnableIRQ(WTIMER5A_IRQn);					//use default priorities
}
...
void wtmr5b_act(void (*isrptr)(void)) {
	...
	NVIC_EnableIRQ(WTIMER5B_IRQn);					//use default priorities
}
