#include "wtimer0.h"									//use timer0

//hardware configuration
//end hardware configuration

//global defines
#define TMRx				WTIMER0					//current timer
...
void wtmr0a_init(uint32_t ps, uint32_t pr) {
	...
	//route the clock
	SYSCTL->RCGCWTIMER |= (1<<0);						//1->route clock. TIMER0/1/2/3/4/5
...
void wtmr0a_act(void (*isrptr)(void)) {
	...
	NVIC_EnableIRQ(WTIMER0A_IRQn);					//use default priorities
}
...
void wtmr0b_act(void (*isrptr)(void)) {
	...
	NVIC_EnableIRQ(WTIMER0B_IRQn);					//use default priorities
}
