#include "wtimer3.h"									//use timer3

//hardware configuration
//end hardware configuration

//global defines
#define TMRx				WTIMER3					//current timer
...
void wtmr3a_init(uint32_t ps, uint32_t pr) {
	...
	//route the clock
	SYSCTL->RCGCWTIMER |= (1<<3);						//1->route clock. TIMER0/1/2/3/4/5
...
void wtmr3a_act(void (*isrptr)(void)) {
	...
	NVIC_EnableIRQ(TIMER3A_IRQn);					//use default priorities
}
...
void wtmr3b_act(void (*isrptr)(void)) {
	...
	NVIC_EnableIRQ(WTIMER3B_IRQn);					//use default priorities
}
