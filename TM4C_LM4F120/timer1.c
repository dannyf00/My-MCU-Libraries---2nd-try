#include "timer1.h"									//use timer1

//hardware configuration
//end hardware configuration

//global defines
#define TMRx				TIMER1					//current timer
...
void tmr1a_init(uint32_t ps, uint32_t pr) {
	...
	//route the clock
	SYSCTL->RCGCTIMER |= (1<<1);						//1->route clock. TIMER0/1/2/3/4/5
...
void tmr1a_act(void (*isrptr)(void)) {
	...
	NVIC_EnableIRQ(TIMER1A_IRQn);					//use default priorities
}
...
void tmr1b_act(void (*isrptr)(void)) {
	...
	NVIC_EnableIRQ(TIMER1B_IRQn);					//use default priorities
}
