#include "timer4.h"									//use timer4

//hardware configuration
//end hardware configuration

//global defines
#define TMRx				TIMER4					//current timer
...
void tmr4a_init(uint32_t ps, uint32_t pr) {
	...
	//route the clock
	SYSCTL->RCGCTIMER |= (1<<4);						//1->route clock. TIMER0/1/2/3/4/5
...
void tmr4a_act(void (*isrptr)(void)) {
	...
	NVIC_EnableIRQ(TIMER4A_IRQn);					//use default priorities
}
...
void tmr4b_act(void (*isrptr)(void)) {
	...
	NVIC_EnableIRQ(TIMER4B_IRQn);					//use default priorities
}
