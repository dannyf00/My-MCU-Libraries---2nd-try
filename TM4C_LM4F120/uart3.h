...
//U3TX=PC7, U3RX=PC6
#define UART3_TX()			do {GPIOC->DEN |= (1<<7); GPIOC->AFSEL |= (1<<7); GPIOC->PCTL = (GPIOC->PCTL &~(0x0f << (7*4))) | (0x01 << (7*4));} while (0)
#define UART3_RX()			do {GPIOC->DEN |= (1<<6); GPIOC->AFSEL |= (1<<6); GPIOC->PCTL = (GPIOC->PCTL &~(0x0f << (6*4))) | (0x01 << (6*4));} while (0)
...
void uart3_init(uint32_t uart_bps);
