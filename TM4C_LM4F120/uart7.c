#include "uart7.h"				//uart0 transmission using interrupt
...
void uart7_init(uint32_t baudrate) {
	...
	//route clock to GPIO
	SYSCTL->RCGCGPIO |= (1<<4);				//0->GPIOA, 1->GPIOB, 2->GPIOC, 3->GPIOD, 4->GPIOE, ...
	while ((SYSCTL->PRGPIO & (1<<4)) == 0) continue;	//1->peripheral ready, 0-> not ready
#if defined(UART7_RX)
	//configure PE0 for alternate function / mux group 1
	UART7_RX();								//GPIOA->DEN |= (1<<0); GPIOA->AFSEL |= (1<<0); GPIOA->PCTL = (GPIOA->PCTL &~(0x0f << (0*4))) | (0x01 << (0*4));
#endif
#if defined(UART7_TX)
	//configure PE1 for alternate function / mux group 1
	UART7_TX();								//GPIOA->DEN |= (1<<1); GPIOA->AFSEL |= (1<<1); GPIOA->PCTL = (GPIOA->PCTL &~(0x0f << (1*4))) | (0x01 << (1*4));
#endif
