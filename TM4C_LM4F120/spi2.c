#include "spi2.h"									//we use spi2

//hardware configuration
#define F_SPI				10000					//spi bit rate
#define SPIx				SSI2					//SSI0..3
...
void spi2_init(void) {
	int tmp;
	//enable clock to ssi
	SYSCTL->RCGCSSI |= (1<<2);						//0->SSI0, 1->SSI1, 2->SSI2, 3->SSI3
	while ((SYSCTL->PRSSI & (1<<2)) == 0) continue;	//1->ready, 0->not ready
	
	//gpio configuration
#if defined(SPI2_SCLK)
	SPI2_SCLK();
#endif
...
uint16_t spi2_write(uint16_t dat) {
	while (spi2_busy()) continue;					//wait for spi0 fifo to free up
