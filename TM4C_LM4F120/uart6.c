#include "uart6.h"				//uart6 transmission using interrupt

//global defines
#define UARTx				UART6
//#define UARTx_IRQn			UART6_IRQn
...
void uart6_init(uint32_t baudrate) {
	...
	UART6_RX();								//GPIOA->DEN |= (1<<0); GPIOA->AFSEL |= (1<<0); GPIOA->PCTL = (GPIOA->PCTL &~(0x0f << (0*4))) | (0x01 << (0*4));
#endif
#if defined(UART6_TX)
	//configure PD5 for alternate function / mux group 1
	UART6_TX();								//GPIOA->DEN |= (1<<1); GPIOA->AFSEL |= (1<<1); GPIOA->PCTL = (GPIOA->PCTL &~(0x0f << (1*4))) | (0x01 << (1*4));
#endif
