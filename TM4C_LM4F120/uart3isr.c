#include "uart3isr.h"				//uart3 transmission using interrupt
...
void uart3_init(uint32_t baudrate) {
	...
	UART3_RX();								//GPIOA->DEN |= (1<<0); GPIOA->AFSEL |= (1<<0); GPIOA->PCTL = (GPIOA->PCTL &~(0x0f << (0*4))) | (0x01 << (0*4));
#endif
#if defined(UART3_TX)
	//configure PC7 for alternate function / mux group 1
	UART3_TX();								//GPIOA->DEN |= (1<<1); GPIOA->AFSEL |= (1<<1); GPIOA->PCTL = (GPIOA->PCTL &~(0x0f << (1*4))) | (0x01 << (1*4));
#endif
