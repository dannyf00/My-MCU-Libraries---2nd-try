#include "timer5.h"									//use timer5

//hardware configuration
//end hardware configuration

//global defines
#define TMRx				TIMER5					//current timer
...
void tmr5a_init(uint32_t ps, uint32_t pr) {
	...
	//route the clock
	SYSCTL->RCGCTIMER |= (1<<5);						//1->route clock. TIMER0/1/2/3/4/5
...
void tmr5a_act(void (*isrptr)(void)) {
	...
	NVIC_EnableIRQ(TIMER5A_IRQn);					//use default priorities
}
...
void tmr5b_act(void (*isrptr)(void)) {
	...
	NVIC_EnableIRQ(TIMER5B_IRQn);					//use default priorities
}
