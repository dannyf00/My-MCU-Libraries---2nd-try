#include "spi1.h"									//we use spi1

//hardware configuration
#define F_SPI				10000					//spi bit rate
#define SPIx				SSI1					//SSI0..3
...
void spi1_init(void) {
	int tmp;
	//enable clock to ssi
	SYSCTL->RCGCSSI |= (1<<1);						//0->SSI0, 1->SSI1, 2->SSI2, 3->SSI3
	while ((SYSCTL->PRSSI & (1<<1)) == 0) continue;	//1->ready, 0->not ready
	
	//gpio configuration
#if defined(SPI1_SCLK)
	SPI1_SCLK();
#endif
...
uint16_t spi1_write(uint16_t dat) {
	while (spi1_busy()) continue;					//wait for spi0 fifo to free up
