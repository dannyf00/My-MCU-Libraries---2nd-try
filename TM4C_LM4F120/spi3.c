#include "spi3.h"									//we use spi3

//hardware configuration
#define F_SPI				10000					//spi bit rate
#define SPIx				SSI3					//SSI0..3
...
void spi3_init(void) {
	int tmp;
	//enable clock to ssi
	SYSCTL->RCGCSSI |= (1<<3);						//0->SSI0, 1->SSI1, 2->SSI2, 3->SSI3
	while ((SYSCTL->PRSSI & (1<<3)) == 0) continue;	//1->ready, 0->not ready
	
	//gpio configuration
#if defined(SPI3_SCLK)
	SPI3_SCLK();
#endif
	
#if defined(SPI3_MOSI)
	SPI3_MOSI();
#endif
	
#if defined(SPI3_MISO)
	SPI3_MISO();
#endif
...
uint16_t spi3_write(uint16_t dat) {
	while (spi3_busy()) continue;					//wait for spi0 fifo to free up
