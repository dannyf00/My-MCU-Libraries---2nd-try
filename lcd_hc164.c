...
#define SPI_CLK	HC164_SCK			//to maintain portability with the spi routines
#define SPI_OUT	HC164_SDO			//LCD_RS line also serves as the SPI_OUT dataline
...
#define spi_send_byte(n)	hc164_write(n)

//replaced by routines in the spi library
/*
void spi_send_byte(unsigned char c) {
    ...
    do  {
		...
		//NOP(); NOP();                      //Ensure minimum delay of 1000ns between bits
    } while (mask != 0);
...
void lcd_init(void) {
	...
	//spi is initialized separately, prior to this code
	//spi_init();						//initialize the spi
	IO_OUT(LCD_DIR, LCD_EN | LCD_RS);	//LCD_RS, LCD_EN, LCD_RS and LCD_CLK in output mode
	LCD_CLR(LCD_EN | LCD_RS);

	delay_ms(15);		// wait 15mSec after power applied,
	spi_send_byte(D4|D5);	//send 0x03
	LCD_STROBE(0);
	...
	spi_send_byte(D5);		//send 0x02 -> 4 bit mode
	LCD_STROBE(0);
