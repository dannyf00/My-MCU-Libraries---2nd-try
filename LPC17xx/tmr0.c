...
#define TMRx			LPC_TIM0
#define TMRx_IRQn		TIMER0_IRQn
...
void tmr0_init(uint32_t prescaler) {
	...
	//enable power
	LPC_SC->PCONP |= (1<<1);		//enable power. 1=tim0, 2=tim1, 22=tim3, 23=tim4
	//enable clock
	//PCLKSEL0: bit 2..3 for timer0, bit 4..5 for timer1
	//PCLKSEL1: bit 12..13 for timer2, bit 14..15 for timer3
	LPC_SC->PCLKSEL0 = (LPC_SC->PCLKSEL0 & ~(0b11<<(2*1))) |		//clear bits
						(0b01 << (2*1));							//0b00->cclk/4, 0b01->cclk, 0b10->cclk/2, 0b11->cclk/8
