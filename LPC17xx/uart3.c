//uar3 driver for LPC17xx

#include "uart3.h"									//we use uart3

//global defines
#define UARTx					LPC_UART3			//specify uart modules used
...
void uart3_init(uint32_t bps) {
	...
	//configure the pins
    // Change P0.0/TXD3 and P0.1/RXD3 mode '0b10'
	LPC_PINCON->PINSEL0 = (LPC_PINCON->PINSEL0 &~(0b11 << 0)) | (0b10 << 0);	//'0b10'->P0.0 to TXD3
	LPC_PINCON->PINSEL0 = (LPC_PINCON->PINSEL0 &~(0b11 << 2)) | (0b10 << 2);	//'0b10'->P0.1 to RXD3

	//enable power to uart
	LPC_SC->PCONP |= (1<<25);							//3=uart0, 4=uart1, 24=uart2, 25=uart3
	//LPC_SC->PCLKSEL0 = (LPC_SC->PCLKSEL0 &~(0b11<<6)) | (0b00 << 6);	//PCLKSEL0: 6=uart0, 8=uart1. 0b00=4:1,0b01=1:1, 0b10=2:1, 0b11=8:1
	LPC_SC->PCLKSEL1 = (LPC_SC->PCLKSEL1 &~(0b11<<18)) | (0b00 << 18);	//PCLKSEL1: 16=uart2, 18=uart3. 0b00=4:1,0b01=1:1, 0b10=2:1, 0b11=8:1

	//find out F_UART
	switch ((LPC_SC->PCLKSEL1 >> 18) & 0b11) {
...
void uart3_put(char ch) {
	while (uart3_busy()) continue;					//wait for transmission to free up
	UARTx->THR = ch;								//start transmission
}

//send a string - \null to terminate
void uart3_puts(char *str) {
	while (*str) uart3_put(*str++);					//keep sending until the end
}
