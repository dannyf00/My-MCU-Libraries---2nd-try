//uar2 driver for LPC17xx

#include "uart2.h"									//we use uart2

//global defines
#define UARTx					LPC_UART2			//specify uart modules used
...
void uart2_init(uint32_t bps) {
	...
	//configure the pins
    // Change P2.8/TXD2 and P2.9/RXD2 mode '0b10'
	LPC_PINCON->PINSEL4 = (LPC_PINCON->PINSEL4 &~(0b11 << 16)) | (0b10 << 16);	//'0b10'->P2.8 to TXD1
	LPC_PINCON->PINSEL4 = (LPC_PINCON->PINSEL4 &~(0b11 << 18)) | (0b10 << 18);	//'0b10'->P2.9 to RXD1

	//enable power to uart
	LPC_SC->PCONP |= (1<<24);							//3=uart0, 4=uart1, 24=uart2, 25=uart3
	...
	//find out F_UART
	switch ((LPC_SC->PCLKSEL1 >> 16) & 0b11) {
...
void uart2_put(char ch) {
	while (uart2_busy()) continue;					//wait for transmission to free up
	UARTx->THR = ch;								//start transmission
}

//send a string - \null to terminate
void uart2_puts(char *str) {
	while (*str) uart2_put(*str++);					//keep sending until the end
}
