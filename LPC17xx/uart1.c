...
void uart1_init(uint32_t bps) {
	...
	//configure the pins
    // Change P2.0/TXD1 and P2.1/RXD1 mode '0b10'
	LPC_PINCON->PINSEL4 = (LPC_PINCON->PINSEL4 &~(0b11 << 0)) | (0b10 << 0);	//'0b10'->P2.0 to TXD1
	LPC_PINCON->PINSEL4 = (LPC_PINCON->PINSEL4 &~(0b11 << 2)) | (0b10 << 2);	//'0b01'->P2.1 to RXD1

	//enable power to uart
	LPC_SC->PCONP |= (1<<4);							//3=uart0, 4=uart1, 24=uart2, 25=uart3
	LPC_SC->PCLKSEL0 = (LPC_SC->PCLKSEL0 &~(0b11<<8)) | (0b00 << 8);	//PCLKSEL0: 6=uart0, 8=uart1. 0b00=4:1,0b01=1:1, 0b10=2:1, 0b11=8:1
	//LPC_SC->PCLKSEL1 = (LPC_SC->PCLKSEL1 &~(0b11<<16)) | (0b00 << 16);	//PCLKSEL1: 16=uart2, 18=uart3. 0b00=4:1,0b01=1:1, 0b10=2:1, 0b11=8:1

	//find out F_UART
	switch ((LPC_SC->PCLKSEL0 >> 8) & 0b11) {
