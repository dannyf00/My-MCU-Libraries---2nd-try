#include "uart.h"								//uart on eUSCI_A0..3 for MSP432

//EUSCI_A

//reset uart0/eusci_a0
void uart_init(UART_TypeDef* eUSCIx, uint32_t bps) {
...
//send a char
void uart_put(UART_TypeDef* eUSCIx, char ch) {
...
//send a string
void uart_puts(UART_TypeDef* eUSCIx, char *str) {
	while (*str) uart_put(eUSCIx, *str++);
}

//receive a char and clear the flag
char uart_get(UART_TypeDef* eUSCIx) {
...
char uart_busy(UART_TypeDef* eUSCIx) {
...
char uart_available(UART_TypeDef* eUSCIx) {
