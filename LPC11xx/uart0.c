//uar0 driver for LPC17xx

#include "uart0.h"									//we use uart0

//global defines
#define UARTx					LPC_UART			//specify uart modules used
...
void uart0_init(uint32_t bps) {
	...
	//configure the pins
    // Change P1.7/TXD0 and P1.6/RXD0 mode '0b001'
	LPC_IOCON->PIO1_7 = (LPC_IOCON->PIO1_7 &~(0b111 <<  0)) | (0b001 <<  0);	//'0b001'->P1.7 to txd
	LPC_IOCON->PIO1_6 = (LPC_IOCON->PIO1_6 &~(0b111 <<  0)) | (0b001 <<  0);	//'0b001'->P1.6 to rxd

	//enable power to uart
	LPC_SYSCON->SYSAHBCLKCTRL |= (1<<12);	//12->uart, 13=adc
	LPC_SYSCON->UARTCLKDIV = 4;				//set uart clock divider. 0->disable uart
	...
	//}
	F_UART = F_CPU / LPC_SYSCON->UARTCLKDIV;
...
void uart0_put(char ch) {
	while (uart0_busy()) continue;					//wait for transmission to free up
	UARTx->THR = ch;								//start transmission
}

//send a string - \null to terminate
void uart0_puts(char *str) {
	while (*str) uart0_put(*str++);					//keep sending until the end
}
