...
//hardware configuration
	//tmr16b1_init(1000);							//set the prescaler
	//tmr16b1_match0(TMR16B_1ms, CTxMR_EINT | CTxMR_RESET);				//set match/reset and enable interrupt = overflow isr
	//tmr16b1_act0(led_set);
	//tmr16b1_match3(TMR16B_1ms / 20, CTxMR_EINT);				//set match and enable interrupt
	//tmr16b1_act3(led_clr);
...
void tmr16b1_match0(uint32_t match, uint32_t config);

//set up match register - ch1
void tmr16b1_match1(uint32_t match, uint32_t config);

//set up match register - ch2
void tmr16b1_match2(uint32_t match, uint32_t config);

//set up match register - ch3
void tmr16b1_match3(uint32_t match, uint32_t config);
...
uint32_t tmr16b1_get0(void);
uint32_t tmr16b1_get1(void);
uint32_t tmr16b1_get2(void);
uint32_t tmr16b1_get3(void);
