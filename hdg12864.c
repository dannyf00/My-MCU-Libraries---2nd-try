...
//offset is 0x00
//
//note: this library does not allow read-back from the lcd
...
#include "gpio.h"
#include "delay.h"
#include "hdg12864.h"				//we use hdg12864 parallel
...
/**********************************************************
               Reset LCD Function
**********************************************************/
void lcd_ctrl_reset(void) {
	lcd_set(lcd_ctrl_cs);				//select the lcd
	lcd_clr(lcd_ctrl_cs);
	lcd_clr(lcd_ctrl_res);			//lcd_ctrl_res low to reset the display. the lcd going into default state.
	delay_ms(1);					//give the lcd time to reset.
	lcd_set(lcd_ctrl_res);			//pulling the lcd out of reset.
}
...
void lcd_init(void) {
   ...
   // the following initialization sequence per datasheet. All needs to be done in 5ms. (X) where X is the command on pg 8-56 of datasheet
	...
	//lcd_clr(lcd_ctrl_cs);						//select the chip
...
// 	delay_us(10);
*/
	//if used with spi routines
	//spi_write(byteforlcd);						//send the byte via spi
	
	//if used with parallel interface
	lcd_clr(lcd_ctrl_cs);
	lcd_data_port = byteforlcd;
	lcd_clr(lcd_ctrl_cs);
...
void lcd_write_command(unsigned char byteforlcd_command) {
	lcd_clr(lcd_ctrl_cs);   							// chip enabled, negative logic. _cs
	lcd_clr(lcd_ctrl_dc);   							// dc=0 signals lcd command mode. A0 in the datasheet. dc=data or command.
	lcd_write_dorc(byteforlcd_command);		//send the command.
	lcd_set(lcd_ctrl_cs);   							// chip disabled
	lcd_clr(lcd_ctrl_cs);   							// strobe out the data
}
...
void lcd_write_data(unsigned char byteforlcd_data) {
	lcd_clr(lcd_ctrl_cs);   							// chip enabled, negative logic. _cs
	lcd_set(lcd_ctrl_dc);   							// display data mode. A0 in the datasheet. dorc=data or command.
