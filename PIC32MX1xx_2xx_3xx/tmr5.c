...
#define _TxON				T5CONbits.TON		//tmr on
#define _TxCS				T5CONbits.TCS		//tmr source
#define _TxGE				T5CONbits.TGATE		//timer gate enable
#define _TxCKPS				T5CONbits.TCKPS		//tmr prescaler
#define _Tx_32				T5CONbits.T32		//32-bit timer if set
#define _TxIE				IEC0bits.T5IE		//tmr interrupt enable
#define _TxIF				IFS0bits.T5IF		//tmr1 overflow flag
#define _TxIP				IPC5bits.T5IP		//t1 interrupt priority
#define _TxIS				IPC5bits.T5IS		//t1 interrupt sub-priority
#define _PRx				PR5					//period
#define _TMRx				TMR5				//counter
...
void __ISR(_TIMER_5_VECTOR, _TxIPL) _T5Interrupt(void) {
...
void tmr5_init(unsigned char ps, unsigned short period) {
