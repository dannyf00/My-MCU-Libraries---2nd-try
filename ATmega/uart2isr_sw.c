#include "uart2isr_sw.h"						//we use software uart / interrupt driven, timer0 as timebase, tx only
	...
	switch (_UxTXphase) {
		...
		case 0:									//end the transmission
			if (*_UxTX_ptr == 0) {				//end of the string has been reached
				...
				TCNT2 = 0;							//reset the timer/prescaler
				//clear TIFR/TIMSK
...
void uart2_init(uint32_t bps) {
	...
	if (F_UART <= bps * 256 * 1) {
		...
		OCR2A = F_UART / bps / 1 - 1;				//set the top
		...
		OCR2A = F_UART / bps / 8 - 1;				//set the top
		...
		OCR2A = F_UART / bps / 32 - 1;				//set the top
		...
		OCR2A = F_UART / bps / 64 - 1;				//set the top
		...
		OCR2A = F_UART / bps / 128 - 1;				//set the top
