#include "uart2bisr_sw.h"						//we use software uart / interrupt driven, timer0 as timebase, tx only

//hardware configuration
#define UART_PORT			PORTB
#define UART_DDR			DDRB
#define UART_TX				(1<<2)			//TX/DO on PB.2
...
ISR(TIMER2_COMPB_vect) {
	...
	switch (_UxTXphase) {
		case 1: 								//first half of the char has been sent, now send the 2nd transmission/data byte
			OCR2B = TCNT2 + _UxTX_PR;			//reset the counter / prescaler
		...
		case 0:									//end the transmission
			if (*_UxTX_ptr == 0) {				//end of the string has been reached
				//USICR = 0;						//disable usi
				//USISR = (1<<USIOIF);
				TIMSK2&=~(1<<OCIE2B);				//1->enable the interrupt, 0->disable the interrupt
				...
				OCR2B = TCNT2 + _UxTX_PR;				//reset the timer/prescaler
				//clear TIFR/TIMSK
...
//reset uart timer0 as clock
//WGM02..0=0b000
//1-start bit, 1-stop bit, 8-data
void uart2b_init(uint32_t bps) {
	...
	//clear TIFR/TIMSK
	TIFR2 |= (1<<OCF2B);					//1->clera the flag
	TIMSK2&=~(1<<OCIE2B);					//1->enable the interrupt, 0->disable the interrupt
...
//send a char
void uart2b_put(char ch) {
	static char _buffer[2]={0,0};			//serial transmission buffer
	_buffer[0]=ch; 							//first char
	uart2b_puts(_buffer);						//send the buffer
}

//send a string - no empty string, null-terminated
void uart2b_puts(char *str) {
	char ch;
	while (uart2b_busy()) continue;			//wait for the uart to be ready
	_UxTX_BUSY = 1;							//uart is busy
	//IO_SET(UART_PORT, UART_TX);			//TX high
	//reverse the input byte's order: LSB..MSB
	_UxTXphase = 1;							//to send the 1st byte
