...
#define UxDLL					U0DLL
#define UxDLM					U0DLM
#define UxFDR          			U0FDR											//not in the header file
...
void uart0_init(uint32_t bps) {
	...
	//configure the pins
    // Change P0.0/TXD0 and P0.1/RXD0 mode '0b01'
	PINSEL0 = (PINSEL0 &~(0x03 << 0)) | (0x01 << 0);	//'0b01'->P0.0 to TXD0
	PINSEL0 = (PINSEL0 &~(0x03 << 2)) | (0x01 << 2);	//'0b01'->P0.1 to RXD0

	//enable power to uart
	PCONP |= (1<<3);							//3=uart0, 4=uart1
...
*/
	F_UART = F_CPU / 4;								//default apbdiv = 4:1
