#include "timer0.h"									//use timer0

//hardware configuration
//end hardware configuration

//global defines
#define TMRx				TIMER0					//current timer
...
//need to insert the following TIMERxA_IntHandler() and TIMERxB_IntHandler() in <device>_startup_ccs.c file
//timera isr handler
#if defined ( __GNUC__ )
void Timer0A_IRQHandler(void) {						//for CoIDE
#else
void TIMER0A_Handler(void) {						//for Keil
#endif
...
//timerb isr handler
#if defined ( __GNUC__ )
void Timer0B_IRQHandler(void) {						//for CoIDE
#else
void TIMER0B_Handler(void) {						//for Keil
#endif
...
void tmr0a_init(uint32_t ps, uint32_t pr) {
	...
	//route the clock
	SYSCTL->RCGC1 |= (1<<16);						//1->route clock. TIMER0/1/2/3/4/5
...
void tmr0a_act(void (*isrptr)(void)) {
	...
	NVIC_EnableIRQ(TIMER0A_IRQn);					//use default priorities
}
...
void tmr0b_act(void (*isrptr)(void)) {
	...
	NVIC_EnableIRQ(TIMER0B_IRQn);					//use default priorities
}
