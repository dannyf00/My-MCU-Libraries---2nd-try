#include "uart0isr.h"				//uart0 transmission using interrupt
...
void uart0_init(uint32_t baudrate) {
	...
	//route clock to GPIO
	SYSCTL->RCGC2 |= (1<<0);				//0->GPIOA, 1->GPIOB, 2->GPIOC, ...
	//while ((SYSCTL->PRGPIO & (1<<0)) == 0) continue;	//1->peripheral ready, 0-> not ready
#if defined(UART0_RX)
	//configure PA0 for alternate function / mux group 1
	UART0_RX();								//GPIOA->DEN |= (1<<0); GPIOA->AFSEL |= (1<<0); GPIOA->PCTL = (GPIOA->PCTL &~(0x0f << (0*4))) | (0x01 << (0*4));
#endif
#if defined(UART0_TX)
	//configure PA1 for alternate function / mux group 1
	UART0_TX();								//GPIOA->DEN |= (1<<1); GPIOA->AFSEL |= (1<<1); GPIOA->PCTL = (GPIOA->PCTL &~(0x0f << (1*4))) | (0x01 << (1*4));
#endif
