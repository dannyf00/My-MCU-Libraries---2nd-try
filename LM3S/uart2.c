...
void uart2_init(uint32_t baudrate) {
	//uart2: tx on PB1, rx on PB0
	//route clock to GPIO
	SYSCTL->RCGC2 |= (1<<1);				//0->GPIOA, 1->GPIOB, 2->GPIOC, ...
	...
	//route clock to UART
	SYSCTL->RCGC1 |= (1<<2);				//0->uart2, 1->uart2, ...
	...
	UARTx->CTL =	(0<<15) |				//0->clear to send disabled, 1->clear to send enabled
					...
					0x00;
	//if (GPIOA->AFSEL & (1<<0)) uart2->CTL |= (1<<9); else uart2->CTL &=~(1<<9);	//enable receiving if PA0 is configured for alternate function
	//if (GPIOA->AFSEL & (1<<1)) uart2->CTL |= (1<<8); else uart2->CTL &=~(1<<8);	//enable transmission if PA1 is configured for alternate function
	...
	//	UARTx->FBRD = (8 * F_UART / baudrate - UART2->IBRD * 64);
	//} else {								//HSE bit cleared -> divide by 16
		//calculate the integer baud rate
		UARTx->IBRD = F_UART / baudrate / 16;
		//calculate the fractional baud rate = 64*fraction
		UARTx->FBRD = (4 * F_UART / baudrate - UART2->IBRD * 64);
	//}
