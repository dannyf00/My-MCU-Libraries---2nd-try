#include "timer2.h"									//use timer2

//hardware configuration
//end hardware configuration

//global defines
#define TMRx				TIMER2					//current timer
...
//timera isr handler
#if defined ( __GNUC__ )
void Timer2A_IRQHandler(void) {						//for CoIDE
#else
void TIMER2A_Handler(void) {						//for Keil
#endif
...
//timerb isr handler
#if defined ( __GNUC__ )
void Timer2B_IRQHandler(void) {						//for CoIDE
#else
void TIMER2B_Handler(void) {						//for Keil
#endif
...
void tmr2a_init(uint32_t ps, uint32_t pr) {
	...
	//route the clock
	SYSCTL->RCGC1 |= (1<<18);						//1->route clock. TIMER0/1/2/3/4/5
...
void tmr2a_act(void (*isrptr)(void)) {
	...
	NVIC_EnableIRQ(TIMER2A_IRQn);					//use default priorities
}
...
void tmr2b_act(void (*isrptr)(void)) {
	...
	NVIC_EnableIRQ(TIMER2B_IRQn);					//use default priorities
}
