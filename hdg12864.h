//header file for hdg12864 using parallel interface

// hardware configuration
// change this section if your pin connection is different
#define lcd_data_port		P2			//lcd connections on PortC
#define lcd_data_dir		P2			//lcd_port's direction register
#define lcd_data_pins		0xff		//all 8 pins are used

#define lcd_ctrl_port		P3			//control signal
#define lcd_ctrl_dir		P3
#define lcd_ctrl_res 		(1<<0)			//lcd_res connected to p0, pin13. Not ncessary as long as it is held high
//#define lcd_sda 			(SPI_MOSI)			//lcd_sda connected to p1, pin14
//#define lcd_sclk 			(SPI_SCK)			//lcd_sclk connected to p2, pin21
#define lcd_ctrl_dc 		(1<<1)			//lcd_dc connected to p3, pin22
#define lcd_ctrl_cs 		(1<<2)			//lcd_cs connected to p4, pin23
#define lcd_ctrl_en			lcd_ctrl_cs		//lcd_ctrl_en = lcd_ctrl_cs
...
#define lcd_set(bits)		IO_SET(lcd_ctrl_port, bits)			//set bit x on lcd_port
#define lcd_clr(bits)		IO_CLR(lcd_ctrl_port, bits)			//clear bit x on lcd_port
#define pin_setup()			{IO_CLR(lcd_ctrl_port, lcd_ctrl_res | lcd_ctrl_dc | lcd_ctrl_cs); IO_OUT(lcd_ctrl_dir, lcd_ctrl_res |/* lcd_sda | lcd_led | lcd_sclk | */lcd_ctrl_dc | lcd_ctrl_cs);	/*set pins for output*/ \
							IO_CLR(lcd_data_port, lcd_data_pins); IO_OUT(lcd_data_dir, lcd_data_pins);	/*data pins as all output*/ \
							}
...
#define LCD_COL_MAX				127							//max column. 95 or 96?
#define LCD_COL_OFFSET			0x00						//first  displayable column. datasheet didn't mention this. WEIRD!
