#include "uartisr_sw.h"					//we use software uart
	...
	switch (_U1TXphase) {
		...
		case 0:									//end the transmission
			if (*_U1TX_ptr == 0) {
				...
				_U1TXDR = *_U1TX_ptr++;							//(r4_lut[ch & 0x0f] << 4) | r4_lut[ch >> 4];
	...
	switch (_U2TXphase) {
		...
		case 0:									//end the transmission
			if (*_U2TX_ptr == 0) {
				...
				//inverse the order of data byte
				_U2TXDR = *_U2TX_ptr++;							//(r4_lut[ch & 0x0f] << 4) | r4_lut[ch >> 4];
				//_UxTXSR = (_UxTXDR << 7) | 0x7f;		//prepare for the next byte
	...
	switch (_U3TXphase) {
		...
		case 0:									//end the transmission
			if (*_U3TX_ptr == 0) {
				...
				//inverse the order of data byte
				_U3TXDR = *_U3TX_ptr++;							//(r4_lut[ch & 0x0f] << 4) | r4_lut[ch >> 4];
				//_UxTXSR = (_UxTXDR << 7) | 0x7f;		//prepare for the next byte
	...
	switch (_U4TXphase) {
		...
		case 0:									//end the transmission
			if (*_U4TX_ptr == 0) {
				...
				//inverse the order of data byte
				_U4TXDR = *_U4TX_ptr++;							//(r4_lut[ch & 0x0f] << 4) | r4_lut[ch >> 4];
				//_UxTXSR = (_UxTXDR << 7) | 0x7f;		//prepare for the next byte
...
void uartoc1_init(uint32_t bps) {
	...
	TIMx->CCMR1 &=~0x0f;				//clear the lowest 4 bits
...
void uartoc1_put(char ch) {
	...
	uartoc1_puts(_buffer);						//send the buffer
...
void uartoc2_init(uint32_t bps) {
	...
	//configure the output compare module
	TIMx->CCMR1 &=~0xf0;				//clear the lowest 4 bits
...
void uartoc3_init(uint32_t bps) {
	...
	//configure the output compare module
	TIMx->CCMR2 &=~0x0f;				//clear the lowest 4 bits
...
void uartoc3_put(char ch) {
	static char _buffer[2]={0,0};			//serial transmission buffer
	_buffer[0]=ch; 							//first char
	uartoc3_puts(_buffer);						//send the buffer
}
...
void uartoc4_init(uint32_t bps) {
	...
	//configure the output compare module
	TIMx->CCMR2 &=~0xf0;				//clear the lowest 4 bits
...
void uartoc4_put(char ch) {
	static char _buffer[2]={0,0};			//serial transmission buffer
	_buffer[0]=ch; 							//first char
	uartoc4_puts(_buffer);						//send the buffer
}
