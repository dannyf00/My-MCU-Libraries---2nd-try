#include "uart1isr.h"								//we use uart
...
void USART1_IRQHandler(void) {
	...
	if (*_UxTX_ptr) {				//not empty, so send it
		UARTx->TDR = *_UxTX_ptr++;	//load up the char, and advance to the next char
	} else {						// come to the end of the string
		UARTx->CR1 &=~(1<<7);		//0->disable the interrupt
		_UxTX_BUSY = 0;				//not busy anymore
	}
}
//reset the uart
//initialize uart1
...
void uart1_puts(char *str) {
	if (*str) {
		_UxTX_BUSY = 1;							//indicate module is busy
		_UxTX_ptr = str;						//point to *str
		UARTx->CR1 |= 1<<7;						//stm32f0xx.h had the CR1/CR2 constants swapped -> don't use them
		UARTx->TDR = *str++;					//load up the data, advance to the next char
	}
	NVIC_EnableIRQ(USART1_IRQn);				//enable irq
	//while (*str) uart1_put(*str++);			//polling version
}
...
uint8_t uart1_busy(void) {
    return _UxTX_BUSY;							//more reliable than txe below
