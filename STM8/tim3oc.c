...
//tim2 isr handler
#pragma vector = TIM3_OVR_UIF_vector		//tim3 overflow interrupt
__interrupt void tim3_ovr_isr(void) {
	TIMx->SR1 &=~(1<<0);						//TIM1_SR_UIF=0;							//clear the flag
	/*_tim2*/_isrptr_tov();							//execute the isr handler
}

//tim2 isr handler - output compare 1..2
#pragma vector = TIM3_CAPCOM_CC1IF_vector		//tim3 overflow interrupt
__interrupt void tim3_oc12_isr(void) {
	uint16_t tmp;
...
void tim3oc1_setpr(uint16_t pr) {
	uint16_t tmp;
...
//activate user handler
void tim3oc1_act(void (*isr_ptr)(void)) {
...
void tim3oc2_setpr(uint16_t pr) {
	uint16_t tmp;
...
//activate user handler
void tim3oc2_act(void (*isr_ptr)(void)) {
