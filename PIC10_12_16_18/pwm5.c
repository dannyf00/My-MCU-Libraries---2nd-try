//#include "gpio.h"
#include "pwm5.h"								//we use pwm1/ccp1

//hardware configuration
//pwm registers
#define PWMx_DDR			TRISA
#define PWMx				(1<<4)
#define PWMx_TMR			PWM_USE_TMR5		//tmr1/3/5 only
...
void pwm5_init(void) {
	...
	CCPxCON = 	0b00000000 |		//PxM1..0
				...
				0b00001100;			//PWM mode
	//PRx = 0xff;						//max period
	CCPRxL = 0xff;					//duty cycle is minimum
	IO_OUT(PWMx_DDR, PWMx);			//turn on the pin drive
}

//set pwm period
void pwm5_setpr(unsigned char prescaler, unsigned short period) {
	//set the period
	switch (_tmr_used) {
		...
		default: 		   CCPxTMR2(prescaler); /*PR1 = period >> 2; */break;
	}
}
