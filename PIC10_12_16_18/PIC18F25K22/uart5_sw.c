#include "uart5_sw.h"						//we use software uart, timer3 as timebase, tx only
...
void uart5_init(uint32_t bps) {
	...
	if (F_UART <= bps * 65536 * 1) {
		...
		//T5CONbits.TOUTPS = 1-1;				//postscaler = 1:1
		...
		T5CONbits.T5CKPS = 3;				//0b11->prescaler = 8:1
