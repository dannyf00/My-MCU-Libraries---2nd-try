#include "uart5isr_sw.h"						//we use software uart / interrupt driven, timer1 as timebase, tx only
...
static uint16_t _UxTXPR=0xffff;					//uart/timer1 period
	...
	switch (_UxTXphase) {
		...
		case 0:									//end the transmission
			if (*_UxTX_ptr == 0) {				//end of the string has been reached
				//USICR = 0;					//disable usi
				//USISR = (1<<USIOIF);
				TMR5IE = 0;						//1->enable the interrupt, 0->disable the interrupt
				...
				TMR5 =-_UxTXPR;								//reset the timer/prescaler
				//clear TIFR/TIMSK
...
//rest uart using usi + timer5 as clock
...
void uart5_init(uint32_t bps) {
	...
	//stop timer2
	TMR5ON = 0;								//0->stop the timer, !0->start the timer
	TMR5 = 0;								//reset the counter

	//clear TIFR/TIMSK
	TMR5IF = 0;								//1->clera the flag
	TMR5IE = 0;								//1->enable the interrupt, 0->disable the interrupt
...
void uart5_put(char ch) {
	...
	uart5_puts(_buffer);						//send the buffer
}

//send a string - no empty string, null-terminated
void uart5_puts(char *str) {
	char ch;
	while (uart5_busy()) continue;			//wait for the uart to be ready
	_UxTX_BUSY = 1;							//uart is busy
	//IO_SET(UART_PORT, UART_TX);			//TX high
	//reverse the input byte's order: LSB..MSB
	_UxTXphase = 1;							//to send the 1st byte
	...
	//clear TIFR/TIMSK
	TMR5IF = 0;								//1->clera the flag
	TMR5IE = 1;								//1->enable the interrupt, 0->disable the interrupt
	IO_CLR(UART_PORT, UART_TX);				//send the start bit -> low
}
