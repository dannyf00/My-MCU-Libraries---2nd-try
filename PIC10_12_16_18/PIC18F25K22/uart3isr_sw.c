#include "uart3isr_sw.h"						//we use software uart / interrupt driven, timer1 as timebase, tx only
	...
	switch (_UxTXphase) {
		...
		case 0:									//end the transmission
			if (*_UxTX_ptr == 0) {				//end of the string has been reached
				//USICR = 0;					//disable usi
				//USISR = (1<<USIOIF);
				TMR3IE = 0;						//1->enable the interrupt, 0->disable the interrupt
				...
				TMR3 =-_UxTXPR;								//reset the timer/prescaler
				//clear TIFR/TIMSK
...
//rest uart using usi + timer1 as clock
...
void uart3_init(uint32_t bps) {
	...
	//stop timer2
	TMR3ON = 0;								//0->stop the timer, !0->start the timer
	TMR3 = 0;								//reset the counter

	//clear TIFR/TIMSK
	TMR3IF = 0;								//1->clera the flag
	TMR3IE = 0;								//1->enable the interrupt, 0->disable the interrupt
...
void uart3_put(char ch) {
	...
	uart3_puts(_buffer);						//send the buffer
}

//send a string - no empty string, null-terminated
void uart3_puts(char *str) {
	char ch;
	while (uart3_busy()) continue;			//wait for the uart to be ready
	_UxTX_BUSY = 1;							//uart is busy
	//IO_SET(UART_PORT, UART_TX);			//TX high
	//reverse the input byte's order: LSB..MSB
	_UxTXphase = 1;							//to send the 1st byte
	...
	//clear TIFR/TIMSK
	TMR3IF = 0;								//1->clera the flag
	TMR3IE = 1;								//1->enable the interrupt, 0->disable the interrupt
	IO_CLR(UART_PORT, UART_TX);				//send the start bit -> low
}
