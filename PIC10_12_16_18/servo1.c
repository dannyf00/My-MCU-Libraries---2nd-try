...
//tmr0: to generate _servo_dc and SERVO_PR
//tmr1: not used
//servo resolution defined by SERVO_MIN

#include <htc.h>						//we use picc
#include "gpio.h"
#include "servo1.h"						//we use servo
...
//unsigned short _servo_pr_counter=0;		//SERVO_PR on counter
...
void interrupt isr(void) {
	if (T0IF) {							//tmr0 to generate SERVO_dly
		...
		if (_servo_counter >= SERVO_PR) {	//total servo period has been reached
			//reset servo counter
			_servo_counter = 0;			//reset servo counter
			//_servo_counter -= SERVO_PR;	//alternative
