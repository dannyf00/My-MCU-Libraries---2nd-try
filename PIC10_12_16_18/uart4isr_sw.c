#include "uart4isr_sw.h"						//we use software uart / interrupt driven, timer2 as timebase, tx only
	...
	switch (_UxTXphase) {
		...
		case 0:									//end the transmission
			if (*_UxTX_ptr == 0) {				//end of the string has been reached
				//USICR = 0;					//disable usi
				//USISR = (1<<USIOIF);
				TMR4IE = 0;						//1->enable the interrupt, 0->disable the interrupt
				...
				TMR4 = 0;							//reset the timer/prescaler
				//clear TIFR/TIMSK
...
void uart4_init(uint32_t bps) {
	...
	//configure timer2: top at PR4
	//stop timer2
	TMR4ON = 0;								//0->stop the timer, !0->start the timer
	TMR4 = 0;								//reset the counter

	//clear TIFR/TIMSK
	TMR4IF = 0;								//1->clera the flag
	TMR4IE = 0;								//1->enable the interrupt, 0->disable the interrupt
	...
	if (F_UART <= bps * 256 * 1) {
		...
		PR4 = F_UART / bps / 1 - 1;			//set the top
		T4CONbits.T4CKPS = 0;				//0b00->prescaler = 1:1
		T4CONbits.T4OUTPS = 1-1;				//postscaler = 1:1
	} else if (F_UART <= bps * 256 * 8) {
		//run at 4:1 prescaler + 2:1 postscaler
		PR4 = F_UART / bps / 8 - 1;		//set the top
		T4CONbits.T4CKPS = 1;				//0b01->prescaler = 4:1
		T4CONbits.T4OUTPS = 2 - 1;			//postscaler = 2:1
	} else if (F_UART <= bps * 256 * 64) {
		//run timer0 at 16:1 prescaler + 4:1 post scaler
		PR4 = F_UART / bps / 64 - 1;		//set the top
		T4CONbits.T4CKPS = 2;				//0b10->prescaler = 16:1
		T4CONbits.T4OUTPS = 4-1;				//postscaler = 4:1
	} else {		
		//run timer2 at 16:1 prescaler + 16:1 postscaler
		PR4 = F_UART / bps / 256 - 1;		//set the top
		T4CONbits.T4CKPS = 2;				//0b10->prescaler = 16:1
		T4CONbits.T4OUTPS = 16-1;			//postscaler = 16:1
	}	
	TMR4ON = 1;								//enable TMR2
	//timer2 running now
...
void uart4_put(char ch) {
	...
	uart4_puts(_buffer);						//send the buffer
}

//send a string - no empty string, null-terminated
void uart4_puts(char *str) {
	char ch;
	while (uart4_busy()) continue;			//wait for the uart to be ready
	_UxTX_BUSY = 1;							//uart is busy
	//IO_SET(UART_PORT, UART_TX);			//TX high
	//reverse the input byte's order: LSB..MSB
	_UxTXphase = 1;							//to send the 1st byte
	...
	//clear TIFR/TIMSK
	TMR4IF = 0;								//1->clera the flag
	TMR4IE = 1;								//1->enable the interrupt, 0->disable the interrupt
	IO_CLR(UART_PORT, UART_TX);				//send the start bit -> low
}
